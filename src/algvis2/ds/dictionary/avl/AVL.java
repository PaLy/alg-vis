...
//			.build();
		...
		Timeline st = TimelineBuilder.create()
			.autoReverse(true)
			.keyFrames(
                    new KeyFrame(Duration.ZERO, kva),
                    new KeyFrame(Duration.millis(1), new KeyValue(node.scaleXProperty(), 3, new MyInter())))
			.build();
...
//                st.getRate();
		...
		final ScaleTransition st2 = ScaleTransitionBuilder.create()
			.node(node2)
			.fromX(node2.getScaleX())
			.toX(3)
			.duration(Duration.millis(500))
			.build();
...
//		sequentialTransition.play();
	...
	class MyInter extends Interpolator {
		...
		public double interpolate(double v, double v1, double v2) {
			System.out.println(v + " " + v1 + " " + v2 + " " + v + (v1 - v) * v2);
			return v + (v1 - v) * v2;
		}
	}
}
