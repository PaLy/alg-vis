...
public class Node extends Group implements AbsPosition, PropertyStateEditable {
    ...
    private void init() {
        ...
        paint.background.addListener(new AutoFillTransition((Shape) getChildren().get(0)));
        paint.text.addListener(new AutoFillTransition((Shape) getChildren().get(1)));
        
        layoutXBindingProperty.addListener(new ChangeListener<DoubleBinding>() {
            @Override
            public void changed(ObservableValue<? extends DoubleBinding> observableValue, DoubleBinding oldBinding, DoubleBinding newBinding) {
//                if (newBinding == null) 
//                    System.out.println(getKey() + " X " + null);
//                else
//                    System.out.println(getKey() + " X " + newBinding.get());
                
                if (newBinding != null) layoutXProperty().bind(newBinding);
                else layoutXProperty().unbind();
                recalcAbsPosition();
            }
        });
...
//                    System.out.println(getKey() + " Y " + newBinding.get());
                ...
                if (newBinding != null) layoutYProperty().bind(newBinding);
                else layoutYProperty().unbind();
                recalcAbsPosition();
            }
        });
        ...
        parentProperty().addListener(new ChangeListener<Parent>() {
            @Override
            public void changed(ObservableValue<? extends Parent> observableValue, Parent parent, Parent parent1) {
                recalcAbsPosition();
            }
        });
	...
	public void goAbove(Node node) {
		...
		layoutYBindingProperty.set(node.visPaneY.add(node.visPaneTranslateY).subtract(Node.RADIUS * 2.4));
	}
